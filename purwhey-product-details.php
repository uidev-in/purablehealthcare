<!DOCTYPE html>
<html lang="en">
    
<?php include 'include/head.php';?>
<style>
.product-details-img {
    margin-right: 160px;
}
.shop-thumb-tab {
    width: 146px;
}

</style>
    <body>
        <!-- header -->
            <?php include 'include/header.php';?>
        <!-- header-end -->

        <!-- Main Content -->
        <main>
            <!-- breadcrumb-area -->
            <section class="breadcrumb-area d-flex align-items-center" style="background-image:url(img/testimonial/test-bg.png)">
                <div class="container">
                    <div class="row align-items-center">
                        <div class="col-xl-12 col-lg-12">
                            <div class="breadcrumb-wrap text-left">
                                <div class="breadcrumb-title mb-30">
                                    <h2>Product Details</h2>    
                                    <div class="breadcrumb-wrap">
                              
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                                        <li class="breadcrumb-item active" aria-current="page">Pricing</li>
                                    </ol>
                                </nav>
                            </div>
                                </div>
                            </div>
                        </div>
                        
                    </div>
                </div>
            </section>
            <!-- breadcrumb-area-end -->

            <!-- shop-banner-area start -->
            <section class="shop-banner-area pt-100 pb-70 " data-animation="fadeInUp animated" data-delay=".2s">
                <div class="container">
                    <div class="row">
                        <div class="col-xl-7">
                        <!-- <div class="shop-thumb-tab mb-30">
                                <ul class="nav" id="myTab2" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab"
                                            aria-selected="true"><img src="img/shop/details/thumb1.jpg" alt=""> </a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="profile-tab" data-toggle="tab" href="#profile" role="tab"
                                            aria-selected="false"><img src="img/shop/details/thumb2.jpg" alt=""></a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="profile-tab2" data-toggle="tab" href="#profile1" role="tab"
                                            aria-selected="false"><img src="img/shop/details/thumb3.jpg" alt=""></a>
                                    </li>
                                </ul>
                            </div> -->
                            <div class="product-details-img mb-30 mr-5">
                                <div class="tab-content" id="myTabContent2">
                                    <div class="tab-pane fade show active" id="home" role="tabpanel">
                                        <div class="product-large-img">
                                            <img src="img/shop/img4.jpg" alt="">
                                        </div>
                                    </div>
                                    <!-- <div class="tab-pane fade" id="profile" role="tabpanel">
                                        <div class="product-large-img">
                                            <img src="img/shop/details/large2.jpg" alt="">
                                        </div>
                                    </div>
                                    <div class="tab-pane fade" id="profile1" role="tabpanel">
                                        <div class="product-large-img">
                                            <img src="img/shop/details/large3.jpg" alt="">
                                        </div>
                                    </div> -->
                                </div>
                            </div>

                           

                        </div>
                        <div class="col-xl-5">
                            <div class="product-details mb-30">
                                <div class="product-details-title">
                                    <p>Category</p>
                                    <h1>Purwhey Powder</h1>
                                    <div class="price details-price pb-30 mb-20">
                                        <span class="text-dark font-weight-bold">Price : </span>
                                        <span class="text-success font-weight-bold"> ₹1199.00</span>
                                        <span class="text-dark"> / 400gm</span>
                                    </div>
                                </div>

                                <h6 class="">Indications : </h6>
                                <ul style="line-height:28px;">
                                    <li><i class="fas fa-check text-success mr-2"></i>Critically ill patients</li>
                                    <li><i class="fas fa-check text-success mr-2"></i>Surgery and Trauma</li>
                                    <li><i class="fas fa-check text-success mr-2"></i>Pulmonary Disease</li>
                                    <li><i class="fas fa-check text-success mr-2"></i>Liver Cirrhosis</li>
                                    <li><i class="fas fa-check text-success mr-2"></i>Patients on ventilator</li>
                                    <li><i class="fas fa-check text-success mr-2"></i>COPD</li>
                                    <li><i class="fas fa-check text-success mr-2"></i>Malabsorption Syndrome</li>
                                </ul>

                            </div>
                        </div>
                    </div>
                </div>
            </section>
            <!-- shop-banner-area end -->

            <!-- product-desc-area start -->
            <section class="product-desc-area pb-55">
                <div class="container">
                    <div class="row">
                        <div class="col-12">
                            <div class="bakix-details-tab">
                                <ul class="nav text-left pb-30 mb-50" id="myTab" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link active" id="desc-tab" data-toggle="tab" href="#id-desc" role="tab"
                                            aria-controls="home" aria-selected="true">Benefits </a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="id-add-in" data-toggle="tab" href="#id-add" role="tab"
                                            aria-controls="profile" aria-selected="false">Additional Information</a>
                                    </li>
                                </ul>
                            </div>
                            <div class="tab-content" id="myTabContent">
                                <div class="tab-pane fade show active" id="id-desc" role="tabpanel" aria-labelledby="desc-tab">
                                    <div class="event-text mb-40">
                                    <ul style="line-height:28px;">
                                        <li>» HBV (High Biological Protien) Goal of Nutrition Support Therapy is to provide maximum calorie and protein in minimum volume</li>
                                        <li>» 40% HBV fullfils incresed protein requirements 1.2g to 2.0g/kg/BW/day. Prevents loss of lean body mass</li>
                                        <li>» Branched chain amino acid helps in maintaing positive balance</li>
                                        <li>» MCT an ideal source of instant energy</li>
                                        <li>» FOS is to promote the growth of beneficial bacteria to support got health </li>
                                        <li>» Vitamins and Minerals speed up recovery and fullfills increased nutritional needs</li>
                                        <li>» Low GI Carbohydrate is safe for diabetic patients</li>
                                        <li>» Purwhey is free from Trans fat, Cholesterol, Lactose and Gluten</li>
                                    </ul>
                                        
                                </div>
                                </div>
                                <div class="tab-pane fade" id="id-add" role="tabpanel" aria-labelledby="id-add-in">
                                <div class="additional-info">
                                        <div class="table-responsive">
                                         <h4>Nutritional Information per 100gm</h4>
                                            <table class="table table table-bordered table-hover text-center">
                                            <thead class="thead-dark">
                                                <tr>
                                                <th scope="col">Nutritional Facts</th>
                                                <th scope="col">Unit</th>
                                                <th scope="col">Per 100 g</th>
                                                <th scope="col">Per 50 g</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                <td>Caloric Content</td>
                                                <td>kcal/kjoules</td>
                                                <td>482/2017</td>
                                                <td>241/1008</td>
                                                </tr>
                                                <tr>
                                                <td>Calories from fat</td>
                                                <td>kcal/kjoules</td>
                                                <td>148/619</td>
                                                <td>74/310</td>
                                                </tr>
                                                <!-- <tr>
                                                    <td colspan="4" >Nutrients</td>
                                                </tr>   -->
                                                <tr>
                                                <td>Total Carbohydrates</td>
                                                <td>g</td>
                                                <td>43.66</td>
                                                <td>21.83</td>
                                                </tr>
                                                <tr>
                                                <td>Sugar (sucrose)</td>
                                                <td>g</td>
                                                <td>0</td>
                                                <td>0</td>
                                                </tr>
                                                <tr>
                                                <td>Dietary Fibre</td>
                                                <td>g</td>
                                                <td>3</td>
                                                <td>1.5</td>
                                                </tr>
                                                <tr>
                                                <td>Fructooligosaccharide</td>
                                                <td>g</td>
                                                <td>3</td>
                                                <td>1.5</td>
                                                </tr>
                                                <tr>
                                                <td>MCT</td>
                                                <td>g</td>
                                                <td>1.64</td>
                                                <td>0.82</td>
                                                </tr>
                                                <tr>
                                                <td>Total Fat</td>
                                                <td>g</td>
                                                <td>16.44</td>
                                                <td>8.22</td>
                                                </tr>
                                                <tr>
                                                <td>Monounsaturated Fatty acid</td>
                                                <td>g</td>
                                                <td>1.28</td>
                                                <td>0.664</td>
                                                </tr>
                                                <tr>
                                                <td>Polyunsaturated Fatty acid</td>
                                                <td>g</td>
                                                <td>2.94</td>
                                                <td>1.47</td>
                                                </tr>
                                                <tr>
                                                <td>Linoleic acid (Omega 6 Fatty Acid)</td>
                                                <td>g</td>
                                                <td>2.86</td>
                                                <td>1.43</td>
                                                </tr>
                                                <tr>
                                                <td>Linoleic acid (Omega 3 Fatty Acid)</td>
                                                <td>g</td>
                                                <td>0.05</td>
                                                <td>0.02</td>
                                                </tr>
                                                <tr>
                                                <td>Saturated Fatty acid</td>
                                                <td>g</td>
                                                <td>12.06</td>
                                                <td>6.03</td>
                                                </tr>
                                                <tr>
                                                <td>Trans Fats</td>
                                                <td>g</td>
                                                <td>0</td>
                                                <td>0</td>
                                                </tr>
                                                <tr>
                                                <td>Cholesterol</td>
                                                <td>mg</td>
                                                <td>0</td>
                                                <td>0</td>
                                                </tr>
                                                <tr>
                                                <td>Probiotic (Lactobacillus Acidophillus)</td>
                                                <td>billion</td>
                                                <td>5</td>
                                                <td>2.5</td>
                                                </tr>
                                                <tr>
                                                <td>Protien</td>
                                                <td>g</td>
                                                <td>40</td>
                                                <td>20</td>
                                                </tr>
                                                <tr>
                                                <td>Leucine</td>
                                                <td>mg</td>
                                                <td>6.51</td>
                                                <td>3.25</td>
                                                </tr>
                                                <tr>
                                                <td>Isoleucine</td>
                                                <td>mg</td>
                                                <td>2.2</td>
                                                <td>1.1</td>
                                                </tr>
                                                <tr>
                                                <td>Valine</td>
                                                <td>mg</td>
                                                <td>17.22</td>
                                                <td>8.61</td>
                                                </tr>
                                                <tr>
                                                <td>Vitamin A</td>
                                                <td>IU</td>
                                                <td>1666.66</td>
                                                <td>833.33</td>
                                                </tr>
                                                <tr>
                                                <td>Vitamin B1</td>
                                                <td>mg</td>
                                                <td>0.66</td>
                                                <td>0.33</td>
                                                </tr>
                                                <tr>
                                                <td>Vitamin B2 </td>
                                                <td>mg</td>
                                                <td>0.72</td>
                                                <td>0.36</td>
                                                </tr>
                                                <tr>
                                                <td>Vitamin B6 </td>
                                                <td>mg</td>
                                                <td>0.94</td>
                                                <td>0.47</td>
                                                </tr>
                                                <tr>
                                                <td>Vitamin B12 </td>
                                                <td>mcg</td>
                                                <td>61.33</td>
                                                <td>0.67</td>
                                                </tr>
                                                <tr>
                                                <td>Vitamin C </td>
                                                <td>mg</td>
                                                <td>50</td>
                                                <td>25</td>
                                                </tr>
                                                <tr>
                                                <td>Vitamin D </td>
                                                <td>IU</td>
                                                <td>333.33</td>
                                                <td>166.67</td>
                                                </tr>
                                                <tr>
                                                <td>Vitamin E </td>
                                                <td>IU</td>
                                                <td>18.33</td>
                                                <td>9.17</td>
                                                </tr>
                                                <tr>
                                                <td>Vitamin K </td>
                                                <td>mcg</td>
                                                <td>66.66</td>
                                                <td>33.33</td>
                                                </tr>
                                                <tr>
                                                <td>Biotin </td>
                                                <td>mcg</td>
                                                <td>16.6</td>
                                                <td>8.33</td>
                                                </tr>
                                                <tr>
                                                <td>Choline </td>
                                                <td>mg</td>
                                                <td>70.37</td>
                                                <td>35.19</td>
                                                </tr>
                                                <tr>
                                                <td>Folic Acid </td>
                                                <td>mcg</td>
                                                <td>222.22</td>
                                                <td>111.11</td>
                                                </tr>
                                                <tr>
                                                <td>Niacinamide </td>
                                                <td>mg</td>
                                                <td>8.88</td>
                                                <td>4.44</td>
                                                </tr>
                                                <tr>
                                                    <td>Pantothenic Acid</td>
                                                    <td>mg</td>
                                                    <td>2.77</td>
                                                    <td>1.39</td>
                                                </tr>
                                                <tr>
                                                    <td>Calcium</td>
                                                    <td>mg</td>
                                                    <td>263.88</td>
                                                    <td>131.94</td>
                                                </tr>
                                                <tr>
                                                <td>Magnesium</td>
                                                <td>mg</td>
                                                <td>233.33</td>
                                                <td>116.67</td>
                                                </tr>

                                                <tr>
                                                <td>Potassium</td>
                                                <td>mg</td>
                                                <td>217.59</td>
                                                <td>108.81</td>
                                                </tr>
                                                <tr>
                                                <td>Chloride</td>
                                                <td>mg</td>
                                                <td>200</td>
                                                <td>100</td>
                                                </tr>
                                                <tr>
                                                <td>Phophorus</td>
                                                <td>mg</td>
                                                <td>131.94</td>
                                                <td>65.97</td>
                                                </tr>
                                                <tr>
                                                <td>Sodium</td>
                                                <td>mg</td>
                                                <td>129.62</td>
                                                <td>64.81</td>
                                                </tr>
                                                <tr>
                                                <td>Iron</td>
                                                <td>mg</td>
                                                <td>10</td>
                                                <td>5</td>
                                                </tr>
                                                <tr>
                                                <td>Zinc</td>
                                                <td>mg</td>
                                                <td>6.11</td>
                                                <td>3.06</td>
                                                </tr>
                                                <tr>
                                                <td>Manganese</td>
                                                <td>mg</td>
                                                <td>1.27</td>
                                                <td>0.64</td>
                                                </tr>
                                                <tr>
                                                <td>Copper</td>
                                                <td>mg</td>
                                                <td>0.5</td>
                                                <td>0.25</td>
                                                </tr>
                                                <tr>
                                                <td>Iodine</td>
                                                <td>mcg</td>
                                                <td>83.33</td>
                                                <td>41.67</td>
                                                </tr>
                                                <tr>
                                                <td>Selenium</td>
                                                <td>mcg</td>
                                                <td>30.55</td>
                                                <td>15.28</td>
                                                </tr>
                                                <tr>
                                                <td>Molybdenum</td>
                                                <td>mcg</td>
                                                <td>25</td>
                                                <td>12.50</td>
                                                </tr>
                                                <tr>
                                                <td>Chromium</td>
                                                <td>mcg</td>
                                                <td>19.44</td>
                                                <td>9.72</td>
                                                </tr>
                                                <tr>
                                                <td>L-Carnitine</td>
                                                <td>mg</td>
                                                <td>46.29</td>
                                                <td>23.15</td>
                                                </tr>
                                                <tr>
                                                <td>L-Taurine</td>
                                                <td>mg</td>
                                                <td>46.29</td>
                                                <td>23.15</td>
                                                </tr>
                                            </tbody>
                                            </table>
                                        </div>
                                    </div>  
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            <!-- product-desc-area end -->

        </main>
        <!-- Main Content end-->

        <!-- footer -->
        <?php include 'include/footer.php';?>
        <!-- footer-end -->

		<!-- JS here -->
        <?php include 'include/script.php';?>
    </body>

</html>

